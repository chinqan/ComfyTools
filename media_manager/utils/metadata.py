...
def get_prompt_summary(meta: dict) -> str:
    """Extract positive/negative prompts from ComfyUI prompt JSON for quick display."""
    ...
    for node_id, node in prompt_data.items():
        ...
        # Common ComfyUI prompt nodes
        if class_type in ("CLIPTextEncode", "CLIPTextEncodeSDXL"):
            text = inputs.get("text", "")
            if text:
                positives.append(str(text))
        elif "negative" in class_type.lower() or "neg" in str(inputs.get("text", "")).lower()[:10]:
            text = inputs.get("text", "")
            if text:
                negatives.append(str(text))

    parts = []
    if positives:
        parts.append("**正面提示詞 (Positive):**\n" + "\n---\n".join(positives[:3]))
